    ...
    <div class="relative pt-28 pb-16 lg:pt-48 lg:pb-32 overflow-hidden bg-white">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-10">
            ...
            <p class="mt-4 text-sm sm:text-lg text-slate-500 max-w-xl mx-auto font-medium mb-8 sm:mb-10 leading-relaxed px-2 sm:px-0">
                Berlokasi di Jalan Ir. Ida Bagus Oka Gang Sundu No.1 <br> Kos Putri Sunduwan menyediakan hunian nyaman dan aman dengan suasana kekeluargaan dan pastinya bikin kamu merasa seperti di rumah sendiri.
            </p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 gap-4 sm:gap-6">
                ...
                <div class="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 inline-flex items-center gap-2 w-max self-start md:self-auto">
                    ...
                    <span class="bg-emerald-100 text-emerald-700 font-black px-2 py-0.5 rounded-lg text-xs sm:text-sm">{{ $kamarKosong->count() }} Kamar</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    ...
                    <a href="{{ route('kamar.detail.public', $kamar->id) }}" class="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transform hover:-translate-y-2 transition duration-500 border border-slate-100 group flex flex-col block cursor-pointer">
                        <div class="h-48 sm:h-56 bg-slate-200 relative overflow-hidden">
                            ...
                            <div class="absolute top-4 right-4 bg-white/90 backdrop-blur-md text-emerald-600 text-[10px] sm:text-xs font-black px-3 py-1.5 sm:px-4 sm:py-2 rounded-full uppercase tracking-wider shadow-sm flex items-center gap-1.5">
                                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500 animate-pulse"></span> Tersedia
                            </div>
                        </div>
                        <div class="p-6 sm:p-8 flex-1 flex flex-col">
                            <p class="text-rose-600 text-xs sm:text-sm font-black uppercase tracking-widest mb-1 border border-rose-200 bg-rose-50 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded w-max">Kamar {{ $kamar->nomor_kamar }}</p>
                            <h3 class="text-lg sm:text-xl font-bold text-slate-900 group-hover:text-rose-600 transition">{{ $kamar->nama_kamar ?? 'Tipe Reguler' }}</h3>
                            <p class="text-slate-500 text-xs sm:text-sm mb-5 sm:mb-6 leading-relaxed line-clamp-2">
                                Klik untuk melihat galeri foto dan detail fasilitas kamar ini.
                            </p>
                ...
                @empty
                    <div class="col-span-full text-center py-16 sm:py-20 bg-white rounded-[2rem] border border-slate-100 shadow-sm mx-4 sm:mx-0">
                        ...
                        <h3 class="text-xl sm:text-2xl font-black text-slate-900 mb-2">Semua Kamar Sedang Penuh</h3>
                        <p class="text-sm sm:text-base text-slate-500 max-w-md mx-auto mb-6 px-4">Saat ini belum ada kamar yang kosong.</p>
                        <a href="https://wa.me/6287876904661" target="_blank" class="inline-flex items-center justify-center px-6 py-3 text-sm font-bold rounded-full text-slate-700 bg-slate-100 hover:bg-slate-200 transition duration-300">
                            Hubungi Pengelola
                        </a>
                    </div>
                @endforelse
    ...
    <footer class="bg-white py-10 sm:py-12 border-t border-slate-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between">
            <div class="flex items-center gap-3 mb-4 md:mb-0">
                <img src="{{ asset('images/logo-sunduwan.png') }}" alt="Logo" class="h-8 sm:h-10 w-auto grayscale opacity-50">
                <span class="font-bold text-slate-500 text-sm sm:text-base">Kos Putri Sunduwan</span>
            </div>
            <div class="text-center md:text-right">
                <p class="text-slate-400 font-medium text-xs sm:text-sm">&copy; {{ date('Y') }} All rights reserved.</p>
                <p class="text-slate-400 text-[10px] sm:text-xs mt-1">Kos Putri Sunduwan</p>
            </div>
        </div>
    </footer>
        ...
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('/sw.js')
                    .then(reg => console.log('Sunduwan PWA Guest: Aktif!'))
